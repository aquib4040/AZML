...
default_desp = {
    ...
    "BASE_URL_PORT": "Which is the BASE_URL Port. Default is 85. Int",
...
MIRROR_HELP_PAGE = """
⌬ <b><u>Mirror & Cloud Drive Commands</u></b>

Mirror commands download files and upload them to Google Drive or RClone cloud storage.

➲ <b>Commands:</b>
• <code>/mirror</code> [link/file] - Mirror link/replied file.
• <code>/qbmirror</code> [magnet/torrent] - Mirror torrents via qBittorrent.
• <code>/ytdl</code> [playlist/video] - Mirror video links via yt-dlp.
• <code>/clone</code> [drive_url] - Copy Google Drive / RClone file or folder.
• <code>/count</code> [drive_url] - Count files and folder size in Google Drive.
• <code>/gdclean</code> [folder_id] - Delete all files in specific Google Drive folder (Sudo only).
• <code>/del</code> [drive_url] - Delete file/folder from Google Drive (Sudo only).

➲ <b>Options/Flags:</b>
Add flags in the command text before download start.
• <code>-up</code> [destination] - Target cloud folder.
• <code>-id</code> [drive_id] - Custom Shared Drive ID.
• <code>-index</code> [url] - Custom GDrive index link.
• <code>-c</code> or <code>-category</code> [name] - Select custom category.
• <code>-zip</code> / <code>-unzip</code> - Zip or unzip archive.
"""

LEECH_HELP_PAGE = """
⌬ <b><u>Telegram Leech Commands</u></b>

Leech commands download files and upload them to Telegram.

➲ <b>Commands:</b>
• <code>/leech</code> [link/file] - Leech link/replied file.
• <code>/qbleech</code> [magnet/torrent] - Leech torrents via qBittorrent.
• <code>/ytdlleech</code> [playlist/video] - Leech videos via yt-dlp.
• <code>/autorename</code> or <code>/ar</code> [template/off] - Set custom AutoRename template.

➲ <b>Settings:</b>
Configure these settings in User Settings menu:
• <b>Prefix & Suffix:</b> Text added to file names.
• <b>Custom Caption:</b> HTML supported Telegram caption.
• <b>Leech Dump:</b> Send files to your private channel.
• <b>Leech Splits:</b> Auto-split files before uploading.
• <b>Personal Upload Bot:</b> Distribute upload task load.

<i>Use the buttons below to read the detailed guides for AutoRename templates and Regex search-and-replace formatting.</i>
"""

USER_HELP_PAGE = """
⌬ <b><u>User Tools & Settings</u></b>

➲ <b>Commands:</b>
• <code>/usetting</code> or <code>/us</code> - Open User settings interface.
• <code>/login</code> [password] - Permanently log in to the bot (Private message).
• <code>/status</code> or <code>/s</code> - Show details of running tasks.
• <code>/stats</code> or <code>/st</code> - Get bot and VPS server performance stats.
• <code>/ping</code> or <code>/p</code> - Test latency between client and bot.
• <code>/rss</code> - Open RSS Feed manager menu.
• <code>/btsel</code> [gid] - Select files from torrent manually.
• <code>/ctsel</code> - Category select menu.
• <code>/cr</code> [link] - Download and scrape Crunchyroll files.
• <code>/scrape</code> [link] - Scrape links from websites.
"""

SEARCH_HELP_PAGE = """
⌬ <b><u>Search & Information Commands</u></b>

➲ <b>Commands:</b>
• <code>/list</code> [query] - Search files inside Google Drive.
• <code>/search</code> [query] - Search torrents using qBittorrent search plugins.
• <code>/imdb</code> [query] - Retrieve detailed movie info from IMDB.
• <code>/anime</code> [query] - Search anime/manga on AniList.
• <code>/mdl</code> [query] - Search Asian dramas on MyDramaList.
• <code>/mediainfo</code> or <code>/mi</code> [url/media] - Get mediainfo stream details of a file.
"""

ADMIN_HELP_PAGE = """
⌬ <b><u>Admin & Sudo Commands (Owner/Sudo Only)</u></b>

➲ <b>Configuration:</b>
• <code>/bsetting</code> or <code>/bs</code> - Configure global bot settings.
• <code>/users</code> - View saved user statistics.
• <code>/authorize</code> or <code>/a</code> - Authorize user or chat.
• <code>/unauthorize</code> or <code>/ua</code> - Unauthorize user or chat.
• <code>/blacklist</code> or <code>/bl</code> - Ban user from bot.
• <code>/rmblacklist</code> or <code>/rbl</code> - Unban user.
• <code>/addsudo</code> / <code>/rmsudo</code> - Add or remove Sudo users.

➲ <b>VPS & Task Control:</b>
• <code>/restart</code> or <code>/r</code> - Restart and update bot.
• <code>/log</code> - Get logs file.
• <code>/cancelall</code> - Cancel all running tasks.
• <code>/broadcast</code> or <code>/bc</code> - Send message to all bot users.

➲ <b>Developers:</b>
• <code>/shell</code> - Run command line shell script.
• <code>/eval</code> - Evaluate python code.
• <code>/exec</code> - Execute python code.
• <code>/clearlocals</code> - Reset evaluation environment.
"""

LEECH_INFO_HELP = """
⌬ <b><u>Telegram Leech Help Guide</u></b>

Leech commands download files from links, torrents, or media and upload them to Telegram.

➲ <b>Commands:</b>
• <code>/leech</code> [link/file] - Leech links or replied files.
• <code>/qbleech</code> [magnet/torrent] - Leech torrents via qBittorrent.
• <code>/ytdlleech</code> [playlist/video] - Leech videos/audio via yt-dlp.

➲ <b>Settings:</b>
• <b>Leech Prefix:</b> Text prepended to all filenames.
• <b>Leech Suffix:</b> Text appended to all filenames.
• <b>Leech Caption:</b> Custom caption/description for Telegram files.
• <b>Leech Dump:</b> Target channel/chat ID where your files will be mirrored.
• <b>Leech Metadata:</b> Edit the default video metadata title/channel name.
• <b>Personal Upload Bot:</b> Distribute your upload tasks using a personal bot token to avoid FloodWaits.

<i>Click the buttons below to read detailed guides on AutoRename and Regex manipulation.</i>
"""

LEECH_AUTORENAME_HELP = """
⌬ <b><u>AutoRename Template Guide</u></b>

AutoRename automatically renames files using templates dynamically parsed from the original filename.

➲ <b>Template Syntax:</b>
Use curly brackets <code>{tag}</code> to extract metadata.

➲ <b>Available Parameters:</b>
• <code>{title}</code> - Title of the video/movie/series.
• <code>{season}</code> - Season number (e.g., 01, 02).
• <code>{episode}</code> - Episode number (e.g., 01, 02).
• <code>{quality}</code> - Video resolution (e.g., 1080p, 720p).
• <code>{codec}</code> - Video codec (e.g., x264, h265, HEVC).
• <code>{audio}</code> - Audio codec/channels (e.g., AAC, DD5.1).

➲ <b>Episode Offsets:</b>
Add offset math inside the tag to shift episode numbering (useful for multi-cour series).
• <code>{episode:+12}</code> - Adds 12 to the parsed episode number (e.g., Ep 01 becomes 13).
• <code>{episode:-5}</code> - Subtracts 5 from the parsed episode number.

➲ <b>Example Template:</b>
<code>{title} S{season}E{episode} [{quality}] [{codec}]</code>
<i>If original file is <code>[Release] Movie.S02E05.1080p.x264.mkv</code>:</i>
<i>It becomes: <code>Movie S02E05 [1080p] [x264].mkv</code></i>
"""

LEECH_REGEX_HELP = """
⌬ <b><u>Regex Remname Guide</u></b>

Regex Rename matches patterns inside the filename and replaces or deletes them.

➲ <b>Regex Syntax:</b>
Use vertical pipes <code>|</code> to separate pattern-replacement pairs.
<b>Format:</b> <code>|pattern:replacement|pattern2:replacement2</code>

➲ <b>Examples:</b>
• <b>Remove specific text:</b>
  <code>|@IndexJunction:</code> (replaces @IndexJunction with empty string).
• <b>Replace spaces with dots:</b>
  <code>| :.</code> (replaces all spaces with dots).
• <b>Remove brackets and their contents:</b>
  <code>|\\[.*?\\].:</code> or <code>|\\[.*?\\]:</code> (removes anything inside square brackets like [AnimeSub]).
• <b>Multiple operations chained:</b>
  <code>|@IndexJunction:||\\s+:.||\\.mkv:.mp4</code>
  (removes username, replaces spaces with dots, and changes extension to .mp4).
"""
