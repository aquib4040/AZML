...
from bot.helper.ext_utils.ffmpeg_utils import resolve_ffmpeg_cmds, proceed_ffmpeg
...
class MirrorLeechListener:
    def __init__(
        ...
        logMessage=None,
        leech_utils={},
        custom_name=False,  # Track if user provided -n custom_name
        ffmpeg_cmds=None,
    ):
        if sameDir is None:
            sameDir = {}
        ...
        self.custom_name = custom_name  # Store custom name flag
    ...
    async def onDownloadComplete(self):
        ...
        if (
            name == "None"
            or self.isQbit
            or self.custom_name  # Force directory listing when -n flag was used
            or not await aiopath.exists(f"{self.dir}/{name}")
        ):
            try:
                files = await listdir(self.dir)
                # Filter out .aria2 incomplete download files
                files = [f for f in files if not f.endswith('.aria2')]
                LOGGER.info(f"DEBUG: Files in download dir: {files}")
            ...
            if name == "yt-dlp-thumb":
                name = files[0]
            LOGGER.info(f"DEBUG: Selected filename from dir: {name}, custom_name={self.custom_name}")

        dl_path = f"{self.dir}/{name}"
        
        if await aiopath.isdir(dl_path):
            for dirpath, _, files_in_dir in await sync_to_async(walk, dl_path):
                for f in files_in_dir:
                    if f.endswith((".aria2", ".!qB")):
                        try:
                            await aioremove(ospath.join(dirpath, f))
                        except:
                            pass
        ...
        if self.extract:
            ...
            if await aiopath.isfile(dl_path):
                dl_path = await fix_archive_extension(dl_path)
                name = ospath.basename(dl_path)
            elif await aiopath.isdir(dl_path):
                for dirpath, _, files in await sync_to_async(walk, dl_path):
                    for f in files:
                        await fix_archive_extension(ospath.join(dirpath, f))
            try:
                if await aiopath.isfile(dl_path):
        ...
        if self.ffmpeg_cmds:
            ...
            if resolved:
                LOGGER.info(f"Running custom FFmpeg commands: {resolved}")
                async with download_dict_lock:
                    download_dict[self.uid] = FFmpegStatus(name, size, gid, self)
                result = await proceed_ffmpeg(self, ffmpeg_path, resolved, gid)
                if result is False:
                    return
                if result:
                    up_path = result
        ...
        # If intro subtitle exists, force metadata processing to apply subtitles
        ...
        stream_options = self.leech_utils.get("stream_options")
        if stream_options and await aiopath.isfile(up_path):
            streams_dir, stream_files = await extract_streams(self, up_path, stream_options)
            if not stream_files:
                await self.onUploadError("Stream extraction failed — no output files produced.")
                return
            total_size = await get_path_size(streams_dir)
            tg = TgUploader(ospath.basename(streams_dir), streams_dir, self)
            tg_status = TelegramStatus(tg, total_size, self.message, gid, "up", self.upload_details)
            async with download_dict_lock:
                download_dict[self.uid] = tg_status
            await update_all_messages()
            await tg.upload([], [], total_size)
            await clean_download(streams_dir)
