...
from bot.helper.ext_utils.personal_bot_manager import PersonalBotManager
...
from bot.helper.ext_utils.tmdb_utils import get_final_poster_url
...
class TgUploader:
    ...
    def __init__(self, name=None, path=None, listener=None):
        ...
        self.__using_personal_bot = False

    def __is_personal_bot_active(self):
        user_dict = user_data.get(self.__user_id, {})
        token = user_dict.get("personal_bot_token")
        enabled = user_dict.get("personal_bot_enabled", False)
        dump = user_dict.get("personal_dump_chat_id")
        dump_enabled = user_dict.get("personal_dump_enabled", False)
        dump_verified = user_dict.get("personal_dump_verified", False)
        return bool(token and enabled and dump and dump_enabled and dump_verified)

    def __get_personal_dump_chat_id(self):
        user_dict = user_data.get(self.__user_id, {})
        chat_id = user_dict.get("personal_dump_chat_id")
        if not chat_id:
            return None
        if isinstance(chat_id, str):
            if chat_id.startswith("-100"):
                try:
                    ...
                    return chat_id
            elif chat_id.startswith("-") or chat_id.isdigit():
                try:
                    return int(chat_id)
                except ValueError:
                    return chat_id
        ...
        try:
            if self.__bot_pm and (
                ...
                try:
                    ...
                    if self.__using_personal_bot:
                        await sendCustomMsg(
                            self.__user_id,
                            f"⚠️ <b>Unable to copy file to PM.</b>\n"
                            f"To receive files directly in your PM when using a Personal Upload Bot, "
                            f"please add the main bot as an Administrator/Member in your Personal Dump Channel."
                        )
                    raise pm_err
    ...
    async def __prepare_file(self, prefile_, dirpath):
        try:
            file_, cap_mono = await format_filename(
                prefile_, 
                self.__user_id, 
                dirpath,
                custom_name=self.__listener.custom_name
            )
        except Exception as err:
    ...
    async def __switching_client(self):
        if self.__prm_media and IS_PREMIUM_USER:
            LOGGER.info("Uploading Media > 2GB by User Client")
            self.__client = user
            self.__using_personal_bot = False
        elif self.__is_personal_bot_active():
            user_dict = user_data.get(self.__user_id, {})
            token = user_dict.get("personal_bot_token")
            try:
                self.__client = await PersonalBotManager.get_instance().get_client(self.__user_id, token)
                self.__using_personal_bot = True
                LOGGER.info("Uploading Media < 2GB by Personal Upload Bot Client")
            except Exception as e:
                LOGGER.error(f"Failed to get Personal Bot Client: {e}. Falling back to main bot.")
                self.__client = bot
                self.__using_personal_bot = False
        else:
            LOGGER.info("Uploading Media < 2GB by Bot Client")
            self.__client = bot
            self.__using_personal_bot = False

    async def __send_media_group(self, subkey, key, msgs):
        if self.__using_personal_bot:
            dump_chat_id = self.__get_personal_dump_chat_id()
            msgs_list = await self.__client.send_media_group(
    ...
    async def __upload_file(self, cap_mono, file, force_document=False):
        ...
        try:
            ...
            if not is_image and thumb is None:
                auto_thumb = user_data.get(self.__user_id, {}).get("auto_thumb", config_dict.get("AUTO_THUMBNAIL", False))
                if auto_thumb:
                    try:
                        rename_regex = config_dict.get("LEECH_FILENAME_REMNAME", "")
                        custom_name = getattr(self.__listener, "custom_name", "")
                        poster_url = await get_final_poster_url(
                            custom_name or file, self.__as_doc, rename_regex
                        )
                        if poster_url:
                            tmdb_thumb = await download_image_url(poster_url)
                            if tmdb_thumb:
                                thumb = tmdb_thumb
                    except Exception as e:
            ...
            if (
                ...
                nrml_media = await self.__client.send_document(
                    chat_id=self.__get_personal_dump_chat_id() if self.__using_personal_bot else self.__sent_msg.chat.id,
                    reply_to_message_id=None if self.__using_personal_bot else self.__sent_msg.id,
                ...
                if not self.__using_personal_bot and self.__prm_media and (self.__has_buttons or not self.__leechmsg):
                    try:
            ...
            if (
                ...
                if (
                    ...
                    and dir_name not in ["Thumbnails", "Images"]
